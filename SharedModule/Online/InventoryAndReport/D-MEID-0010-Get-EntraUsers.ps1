...
.EXAMPLE
    .\SM-IR3001-Get-EntraUsers.ps1 -InputCsvPath .\3001.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3001-Get-EntraUsers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-MEID-0010-Get-EntraUsers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
