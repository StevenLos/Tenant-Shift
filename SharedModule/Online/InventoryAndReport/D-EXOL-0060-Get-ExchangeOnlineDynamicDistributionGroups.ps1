...
.EXAMPLE
    .\SM-IR3123-Get-ExchangeOnlineDynamicDistributionGroups.ps1 -InputCsvPath .\3123.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3123-Get-ExchangeOnlineDynamicDistributionGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0060-Get-ExchangeOnlineDynamicDistributionGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
