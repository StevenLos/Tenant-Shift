...
.EXAMPLE
    .\SM-D3204-Get-OneDriveProvisioningStatus.ps1 -InputCsvPath .\3204.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3204-Get-OneDriveProvisioningStatus.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-ONDR-0010-Get-OneDriveProvisioningStatus_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
