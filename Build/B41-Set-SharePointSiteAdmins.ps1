...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path $PSScriptRoot -ChildPath ("Results_B41-Set-SharePointSiteAdmins_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting SharePoint site admin assignment script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($PSCmdlet.ShouldProcess($siteUrl, 'Set SharePoint site collection administrators')) {
...
Write-Status -Message 'SharePoint site admin assignment script completed.' -Level SUCCESS
