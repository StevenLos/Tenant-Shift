...
.SYNOPSIS
    Modifies OneDriveSiteLockState in Microsoft 365.

.DESCRIPTION
    Updates OneDriveSiteLockState in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3209-Set-OneDriveSiteLockState.ps1 -InputCsvPath .\3209.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3209-Set-OneDriveSiteLockState.ps1 -InputCsvPath .\3209.input.csv -WhatIf
...
.NOTES
    ...
    LockState             String    Yes       <fill in description>
#>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3209-Set-OneDriveSiteLockState_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting OneDrive site lock state update script.'
...
if (-not $setSpoSiteCommand.Parameters.ContainsKey('LockState')) {
    throw "Set-SPOSite parameter '-LockState' is not available in the installed Microsoft.Online.SharePoint.PowerShell module."
}
...
foreach ($row in $rows) {
    ...
    try {
        ...
        $lockStateRaw = ([string]$row.LockState).Trim()
        if ([string]::IsNullOrWhiteSpace($lockStateRaw)) {
            throw 'LockState is required.'
        }
        ...
        if ($resolution.Status -eq 'NotFound') {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'NotFound' -Message "$($resolution.Message) Expected URL: $($resolution.ExpectedSiteUrl)"))
            $rowNumber++
            continue
        }

        if ($resolution.Status -eq 'Ambiguous') {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'Failed' -Message "$($resolution.Message) Expected URL: $($resolution.ExpectedSiteUrl)"))
            $rowNumber++
            continue
        }
        ...
        $lockStateValue = ConvertTo-SetSpoParameterValue -ParameterMetadata $setSpoSiteCommand.Parameters['LockState'] -RawValue $lockStateRaw -FieldName 'LockState'
        $lockStateText = [string]$lockStateValue

        $currentLockState = ''
        if ($site.PSObject.Properties.Name -contains 'LockState') {
            $currentLockState = ([string]$site.LockState).Trim()
        }

        if (-not [string]::IsNullOrWhiteSpace($currentLockState) -and $currentLockState.Equals($lockStateText, [System.StringComparison]::OrdinalIgnoreCase)) {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'Skipped' -Message "OneDrive lock state already set to '$currentLockState'."))
            $rowNumber++
            continue
        }

        if ($PSCmdlet.ShouldProcess($siteUrl, "Set OneDrive lock state to '$lockStateText'")) {
            Invoke-WithRetry -OperationName "Set OneDrive lock state $siteUrl" -ScriptBlock {
                Set-SPOSite -Identity $siteUrl -LockState $lockStateValue -ErrorAction Stop
            }

            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'Completed' -Message "OneDrive lock state updated to '$lockStateText'."))
        }
        else {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'WhatIf' -Message 'OneDrive lock state update skipped due to WhatIf.'))
        }
    }
    catch {
        Write-Status -Message "Row $rowNumber ($userPrincipalName) failed: $($_.Exception.Message)" -Level ERROR
        $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'SetOneDriveSiteLockState' -Status 'Failed' -Message $_.Exception.Message))
    }
...
Write-Status -Message 'OneDrive site lock state update script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
