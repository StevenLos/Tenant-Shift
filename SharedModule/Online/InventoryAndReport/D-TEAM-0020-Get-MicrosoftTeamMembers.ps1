...
.SYNOPSIS
    Gets MicrosoftTeamMembers and exports results to CSV.

.DESCRIPTION
    Gets MicrosoftTeamMembers from Microsoft 365 and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR3310-Get-MicrosoftTeamMembers.ps1 -InputCsvPath .\3310.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3310-Get-MicrosoftTeamMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-TEAM-0020-Get-MicrosoftTeamMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
