...
.SYNOPSIS
    Modifies EntraSecurityGroupMembers in Microsoft 365.

.DESCRIPTION
    Updates EntraSecurityGroupMembers in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3007-Set-EntraSecurityGroupMembers.ps1 -InputCsvPath .\3007.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3007-Set-EntraSecurityGroupMembers.ps1 -InputCsvPath .\3007.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3007-Set-EntraSecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
