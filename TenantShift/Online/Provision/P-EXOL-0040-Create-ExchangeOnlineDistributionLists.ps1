...
.SYNOPSIS
    Provisions ExchangeOnlineDistributionLists in Microsoft 365.

.DESCRIPTION
    Creates ExchangeOnlineDistributionLists in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P3114-Create-ExchangeOnlineDistributionLists.ps1 -InputCsvPath .\3114.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P3114-Create-ExchangeOnlineDistributionLists.ps1 -InputCsvPath .\3114.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P3114-Create-ExchangeOnlineDistributionLists_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
