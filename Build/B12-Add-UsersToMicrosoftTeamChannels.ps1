...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path $PSScriptRoot -ChildPath ("Results_B12-Add-UsersToMicrosoftTeamChannels_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($membershipType.Equals('Standard', [System.StringComparison]::OrdinalIgnoreCase)) {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey "$teamMailNickname|$channelDisplayName|$upn" -Action 'SetTeamChannelMembership' -Status 'Skipped' -Message 'Standard channels inherit Team membership. Use B10-Add-UsersToMicrosoftTeams.ps1 to manage access.'))
            $rowNumber++
            continue
        }
