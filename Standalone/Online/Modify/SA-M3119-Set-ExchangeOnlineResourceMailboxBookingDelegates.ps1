...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-M3119-Set-ExchangeOnlineResourceMailboxBookingDelegates_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
                ...
                if ($delegateRaw -eq '-') {
                    ...
                    foreach ($delegateUpn in $delegateUpns) {
                        $delegate = Invoke-WithRetry -OperationName "Resolve delegate $delegateUpn" -ScriptBlock {
                            Get-EXORecipient -Identity $delegateUpn -ErrorAction Stop
                        }
