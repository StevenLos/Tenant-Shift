...
.EXAMPLE
    .\SM-D0005-Get-ActiveDirectorySecurityGroups.ps1 -InputCsvPath .\0005.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0005-Get-ActiveDirectorySecurityGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0005-Get-ActiveDirectorySecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
