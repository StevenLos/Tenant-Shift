...
.EXAMPLE
    .\SM-IR3127-Get-ExchangeOnlineMailboxSizes.ps1 -InputCsvPath .\3127.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3127-Get-ExchangeOnlineMailboxSizes.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0230-Get-ExchangeOnlineMailboxSizes_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
