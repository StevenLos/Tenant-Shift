...
.EXAMPLE
    .\SM-D3132-Get-ExchangeOnlineSharedMailboxSmtpAddresses.ps1 -InputCsvPath .\3132.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3132-Get-ExchangeOnlineSharedMailboxSmtpAddresses.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0270-Get-ExchangeOnlineSharedMailboxSmtpAddresses_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
