...
.SYNOPSIS
    Modifies ExchangeOnlineSharedMailboxPermissions in Microsoft 365.

.DESCRIPTION
    Updates ExchangeOnlineSharedMailboxPermissions in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3117-Set-ExchangeOnlineSharedMailboxPermissions.ps1 -InputCsvPath .\3117.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3117-Set-ExchangeOnlineSharedMailboxPermissions.ps1 -InputCsvPath .\3117.input.csv -WhatIf
...
.NOTES
    ...
    TrusteeUserPrincipalName  String    Yes       <fill in description>
    FullAccess                String    Yes       <fill in description>
    ReadOnly                  String    Yes       <fill in description>
    SendAs                    String    Yes       <fill in description>
    SendOnBehalf              String    Yes       <fill in description>
    AutoMapping               String    Yes       <fill in description>
#>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3117-Set-ExchangeOnlineSharedMailboxPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Exchange Online shared mailbox permission assignment script.'
...
Write-Status -Message 'Exchange Online shared mailbox permission assignment script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
