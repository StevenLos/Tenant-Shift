...
.SYNOPSIS
    Gets ActiveDirectorySecurityGroupMembers and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectorySecurityGroupMembers from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0007-Get-ActiveDirectorySecurityGroupMembers.ps1 -InputCsvPath .\0007.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0007-Get-ActiveDirectorySecurityGroupMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0007-Get-ActiveDirectorySecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Active Directory security group membership inventory script.'
...
if ($PSCmdlet.ParameterSetName -eq 'DiscoverAll') {
    ...
    Write-Status -Message "DiscoverAll enabled for Active Directory group members. SearchBase='$resolvedSearchBase'." -Level WARN
    $rows = @([PSCustomObject]@{
            IdentityType  = 'All'
            IdentityValue = '*'
        })
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($groups.Count -eq 0) {
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'GetActiveDirectorySecurityGroupMembers' -Status 'NotFound' -Message 'No matching groups were found.' -Data ([ordered]@{
        ...
        foreach ($group in @($groups | Sort-Object -Property SamAccountName, Name, DistinguishedName)) {
            ...
            if ($members.Count -eq 0) {
                $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $groupKey -Action 'GetActiveDirectorySecurityGroupMembers' -Status 'Completed' -Message 'Group has no direct members.' -Data ([ordered]@{
                                IdentityTypeRequested = $identityType
                                IdentityValueRequested = $identityValue
            ...
            foreach ($member in @($members | Sort-Object -Property objectClass, Name, DistinguishedName)) {
                ...
                $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $resultKey -Action 'GetActiveDirectorySecurityGroupMembers' -Status 'Completed' -Message 'Group member exported.' -Data ([ordered]@{
                                IdentityTypeRequested = $identityType
                                IdentityValueRequested = $identityValue
                                GroupDistinguishedName = Get-TrimmedValue -Value $group.DistinguishedName
                                GroupSamAccountName = Get-TrimmedValue -Value $group.SamAccountName
        ...
        $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'GetActiveDirectorySecurityGroupMembers' -Status 'Failed' -Message $_.Exception.Message -Data ([ordered]@{
                        IdentityTypeRequested = $identityType
                        IdentityValueRequested = $identityValue
                        GroupDistinguishedName = ''
                        GroupSamAccountName = ''
...
Write-Status -Message 'Active Directory security group membership inventory script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
