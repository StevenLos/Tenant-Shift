...
.EXAMPLE
    .\SM-IR3122-Get-ExchangeOnlineMailEnabledSecurityGroups.ps1 -InputCsvPath .\3122.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3122-Get-ExchangeOnlineMailEnabledSecurityGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0050-Get-ExchangeOnlineMailEnabledSecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
