...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-M3117-Set-ExchangeOnlineSharedMailboxPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
        ...
        if ($grantFullAccess) {
            try {
                $existingFullAccess = Invoke-WithRetry -OperationName "Check FullAccess $mailboxIdentity -> $trusteeUpn" -ScriptBlock {
                    Get-EXOMailboxPermission -Identity $mailbox.Identity -User $trustee.Identity -ErrorAction SilentlyContinue |
        ...
        if ($grantSendAs) {
            try {
                $existingSendAs = Invoke-WithRetry -OperationName "Check SendAs $mailboxIdentity -> $trusteeUpn" -ScriptBlock {
                    Get-EXORecipientPermission -Identity $mailbox.Identity -Trustee $trustee.Identity -ErrorAction SilentlyContinue |
