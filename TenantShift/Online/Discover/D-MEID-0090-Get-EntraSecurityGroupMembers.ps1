...
.EXAMPLE
    .\SM-D3007-Get-EntraSecurityGroupMembers.ps1 -InputCsvPath .\3007.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3007-Get-EntraSecurityGroupMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-MEID-0090-Get-EntraSecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
