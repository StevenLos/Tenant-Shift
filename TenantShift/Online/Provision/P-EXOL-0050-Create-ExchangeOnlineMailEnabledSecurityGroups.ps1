...
.SYNOPSIS
    Provisions ExchangeOnlineMailEnabledSecurityGroups in Microsoft 365.

.DESCRIPTION
    Creates ExchangeOnlineMailEnabledSecurityGroups in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P3115-Create-ExchangeOnlineMailEnabledSecurityGroups.ps1 -InputCsvPath .\3115.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P3115-Create-ExchangeOnlineMailEnabledSecurityGroups.ps1 -InputCsvPath .\3115.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P3115-Create-ExchangeOnlineMailEnabledSecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
