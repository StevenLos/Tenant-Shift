...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR3312-Get-MicrosoftTeamChannelMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Ensure-GraphConnection -RequiredScopes @('Group.Read.All', 'Team.ReadBasic.All', 'TeamSettings.Read.All', 'TeamMember.Read.All', 'Directory.Read.All')
