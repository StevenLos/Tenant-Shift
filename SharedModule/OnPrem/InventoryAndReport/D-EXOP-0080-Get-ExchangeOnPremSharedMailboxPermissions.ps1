...
.SYNOPSIS
    Gets ExchangeOnPremSharedMailboxPermissions and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremSharedMailboxPermissions from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0217-Get-ExchangeOnPremSharedMailboxPermissions.ps1 -InputCsvPath .\0217.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0217-Get-ExchangeOnPremSharedMailboxPermissions.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0217-Get-ExchangeOnPremSharedMailboxPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Exchange on-prem shared mailbox permission inventory script.'
...
if ($PSCmdlet.ParameterSetName -eq 'DiscoverAll') {
    ...
    Write-Status -Message "DiscoverAll enabled for Exchange on-prem shared mailbox permissions. SearchBase='$resolvedSearchBase'." -Level WARN
    $rows = @([PSCustomObject]@{
            SharedMailboxIdentity = '*'
        })
}
...
Write-Status -Message 'Exchange on-prem shared mailbox permission inventory script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
