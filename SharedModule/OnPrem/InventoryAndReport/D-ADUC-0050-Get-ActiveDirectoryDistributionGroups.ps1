...
.SYNOPSIS
    Gets ActiveDirectoryDistributionGroups and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectoryDistributionGroups from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0006-Get-ActiveDirectoryDistributionGroups.ps1 -InputCsvPath .\0006.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0006-Get-ActiveDirectoryDistributionGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0006-Get-ActiveDirectoryDistributionGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
