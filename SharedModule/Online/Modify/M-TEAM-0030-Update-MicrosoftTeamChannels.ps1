...
.SYNOPSIS
    Modifies MicrosoftTeamChannels in Microsoft 365.

.DESCRIPTION
    Updates MicrosoftTeamChannels in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3311-Update-MicrosoftTeamChannels.ps1 -InputCsvPath .\3311.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3311-Update-MicrosoftTeamChannels.ps1 -InputCsvPath .\3311.input.csv -WhatIf
...
.NOTES
    ...
    MembershipType           String    Yes       <fill in description>
    OwnerUserPrincipalNames  String    Yes       <fill in description>
    IsFavoriteByDefault      String    Yes       <fill in description>
#>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3311-Update-MicrosoftTeamChannels_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
