...
.SYNOPSIS
    Modifies ExchangeOnlineResourceMailboxBookingDelegates in Microsoft 365.

.DESCRIPTION
    Updates ExchangeOnlineResourceMailboxBookingDelegates in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3119-Set-ExchangeOnlineResourceMailboxBookingDelegates.ps1 -InputCsvPath .\3119.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3119-Set-ExchangeOnlineResourceMailboxBookingDelegates.ps1 -InputCsvPath .\3119.input.csv -WhatIf
...
.NOTES
    ...
    DelegateUserPrincipalNames   String    Yes       <fill in description>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3119-Set-ExchangeOnlineResourceMailboxBookingDelegates_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
                ...
                if ($delegateRaw -eq '-') {
                    ...
                    foreach ($delegateUpn in $delegateUpns) {
                        $delegate = Invoke-WithRetry -OperationName "Resolve delegate $delegateUpn" -ScriptBlock {
                            Get-ExchangeOnlineRecipient -Identity $delegateUpn -ErrorAction Stop
                        }
