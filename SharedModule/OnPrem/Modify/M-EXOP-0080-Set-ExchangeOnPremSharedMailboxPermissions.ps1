...
.SYNOPSIS
    Modifies ExchangeOnPremSharedMailboxPermissions in Active Directory.

.DESCRIPTION
    Updates ExchangeOnPremSharedMailboxPermissions in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0217-Set-ExchangeOnPremSharedMailboxPermissions.ps1 -InputCsvPath .\0217.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0217-Set-ExchangeOnPremSharedMailboxPermissions.ps1 -InputCsvPath .\0217.input.csv -WhatIf
...
.NOTES
    ...
    PermissionType         String    Yes       <fill in description>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0217-Set-ExchangeOnPremSharedMailboxPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
