...
.SYNOPSIS
    Provisions ExchangeOnPremDistributionLists in Active Directory.

.DESCRIPTION
    Creates ExchangeOnPremDistributionLists in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P0214-Create-ExchangeOnPremDistributionLists.ps1 -InputCsvPath .\0214.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P0214-Create-ExchangeOnPremDistributionLists.ps1 -InputCsvPath .\0214.input.csv -WhatIf
...
.NOTES
    ...
    MemberJoinRestriction                 String    Yes       <fill in description>
    MemberDepartRestriction               String    Yes       <fill in description>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P0214-Create-ExchangeOnPremDistributionLists_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Exchange on-prem distribution list creation script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($existingGroup) {
            ...
            if ($recipientTypeDetails -eq 'MailUniversalDistributionGroup' -or $recipientTypeDetails -eq 'MailNonUniversalGroup') {
        ...
        if ($PSCmdlet.ShouldProcess($identityToCheck, 'Create Exchange on-prem distribution list')) {
...
Write-Status -Message 'Exchange on-prem distribution list creation script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
