...
.SYNOPSIS
    Modifies ActiveDirectorySecurityGroupMembers in Active Directory.

.DESCRIPTION
    Updates ActiveDirectorySecurityGroupMembers in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0007-Set-ActiveDirectorySecurityGroupMembers.ps1 -InputCsvPath .\0007.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0007-Set-ActiveDirectorySecurityGroupMembers.ps1 -InputCsvPath .\0007.input.csv -WhatIf
...
.NOTES
    ...
    GroupIdentityType     String    Yes       <fill in description>
    GroupIdentityValue    String    Yes       <fill in description>
    MemberIdentityType    String    Yes       <fill in description>
    MemberIdentityValue   String    Yes       <fill in description>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0007-Set-ActiveDirectorySecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Active Directory security group membership update script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($memberAction -eq 'add') {
            if ($memberExists) {
                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'Skipped' -Message 'Member already present in group.'))
            }
            elseif ($PSCmdlet.ShouldProcess($targetGroup.SamAccountName, "Add member $memberDn")) {
                Invoke-WithRetry -OperationName "Add member to AD security group $($targetGroup.SamAccountName)" -ScriptBlock {
                    Add-ADGroupMember -Identity $targetGroup.ObjectGuid -Members $memberDn -ErrorAction Stop
                }

                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'Completed' -Message 'Member added to group.'))
            }
            else {
                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'WhatIf' -Message 'Add skipped due to WhatIf.'))
            }
        }
        else {
            if (-not $memberExists) {
                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'Skipped' -Message 'Member is not currently in group.'))
            }
            elseif ($PSCmdlet.ShouldProcess($targetGroup.SamAccountName, "Remove member $memberDn")) {
                Invoke-WithRetry -OperationName "Remove member from AD security group $($targetGroup.SamAccountName)" -ScriptBlock {
                    Remove-ADGroupMember -Identity $targetGroup.ObjectGuid -Members $memberDn -Confirm:$false -ErrorAction Stop
                }

                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'Completed' -Message 'Member removed from group.'))
            }
            else {
                $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'WhatIf' -Message 'Remove skipped due to WhatIf.'))
            }
        }
    }
    catch {
        Write-Status -Message "Row $rowNumber ($primaryKey) failed: $($_.Exception.Message)" -Level ERROR
        $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'SetActiveDirectorySecurityGroupMember' -Status 'Failed' -Message $_.Exception.Message))
    }
...
Write-Status -Message 'Active Directory security group membership update script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
