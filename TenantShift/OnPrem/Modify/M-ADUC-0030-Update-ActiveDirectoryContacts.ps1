...
.SYNOPSIS
    Modifies ActiveDirectoryContacts in Active Directory.

.DESCRIPTION
    Updates ActiveDirectoryContacts in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0002-Update-ActiveDirectoryContacts.ps1 -InputCsvPath .\0002.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0002-Update-ActiveDirectoryContacts.ps1 -InputCsvPath .\0002.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0002-Update-ActiveDirectoryContacts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
