...
.SYNOPSIS
    Gets ExchangeOnPremDistributionListMembers and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremDistributionListMembers from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0215-Get-ExchangeOnPremDistributionListMembers.ps1 -InputCsvPath .\0215.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0215-Get-ExchangeOnPremDistributionListMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0215-Get-ExchangeOnPremDistributionListMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
