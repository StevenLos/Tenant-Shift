...
.EXAMPLE
    .\SM-D3003-Get-EntraUserLicenses.ps1 -InputCsvPath .\3003.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3003-Get-EntraUserLicenses.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-MEID-0030-Get-EntraUserLicenses_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
