...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-M3126-Set-ExchangeOnlineMailboxAllowedAuthenticationMethods_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        $casMailbox = Invoke-WithRetry -OperationName "Lookup CAS mailbox $mailboxIdentity" -ScriptBlock {
            Get-EXOCasMailbox -Identity $mailbox.Identity -ErrorAction Stop
        }
