# Operator Runbook — OnPrem InventoryAndReport Scripts
...
OnPrem IR scripts query Active Directory and on-premises Exchange. Required permissions:

| Workload | Minimum permission |
|----------|--------------------|
| Active Directory (AD) | Domain Users (read access is sufficient for most IR scripts); some scripts require delegation to read confidential attributes |
...
> **Note:** Active Directory uses multi-master replication. Changes made by Provision or Modify scripts may not be visible immediately on all domain controllers. If you are running an IR script to verify a recent Provision or Modify operation, allow sufficient time for replication (typically 15 seconds within a site; up to 3 hours across sites for the default schedule). Use the `-Server` parameter to target a specific domain controller if you need to read from the DC where the change was written.
...
```
.\SharedModule\OnPrem\InventoryAndReport\D-ADUC-0020-Get-ActiveDirectoryUsers.ps1
.\SharedModule\OnPrem\InventoryAndReport\Scope-ActiveDirectoryUsers.input.csv    ← template
```
...
```
.\SharedModule\OnPrem\InventoryAndReport\InventoryAndReport_OutputCsvPath\
```
