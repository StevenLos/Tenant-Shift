...
.SYNOPSIS
    Gets ExchangeOnPremResourceMailboxBookingDelegates and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremResourceMailboxBookingDelegates from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0219-Get-ExchangeOnPremResourceMailboxBookingDelegates.ps1 -InputCsvPath .\0219.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0219-Get-ExchangeOnPremResourceMailboxBookingDelegates.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0219-Get-ExchangeOnPremResourceMailboxBookingDelegates_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Exchange on-prem resource mailbox booking delegate inventory script.'
...
if ($PSCmdlet.ParameterSetName -eq 'DiscoverAll') {
    ...
    Write-Status -Message "DiscoverAll enabled for Exchange on-prem resource mailbox booking delegates. SearchBase='$resolvedSearchBase'." -Level WARN
    $rows = @([PSCustomObject]@{
            ResourceMailboxIdentity = '*'
        })
}
    ...
    try {
        ...
        foreach ($mailbox in @($mailboxes | Sort-Object -Property DisplayName, Identity)) {
            ...
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $identity -Action 'GetExchangeResourceMailboxBookingDelegates' -Status 'Completed' -Message 'Resource mailbox booking settings exported.' -Data ([ordered]@{
                        ...
                        AutomateProcessing          = Get-TrimmedValue -Value $calendar.AutomateProcessing
...
Write-Status -Message 'Exchange on-prem resource mailbox booking delegate inventory script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
