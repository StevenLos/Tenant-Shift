...
.EXAMPLE
    .\SM-D3113-Get-ExchangeOnlineMailContacts.ps1 -InputCsvPath .\3113.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3113-Get-ExchangeOnlineMailContacts.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0030-Get-ExchangeOnlineMailContacts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
