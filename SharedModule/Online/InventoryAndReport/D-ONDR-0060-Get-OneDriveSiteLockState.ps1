...
.SYNOPSIS
    Gets OneDriveSiteLockState and exports results to CSV.

.DESCRIPTION
    Gets OneDriveSiteLockState from Microsoft 365 and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR3209-Get-OneDriveSiteLockState.ps1 -InputCsvPath .\3209.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3209-Get-OneDriveSiteLockState.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-ONDR-0060-Get-OneDriveSiteLockState_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting OneDrive lock state inventory script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($resolvedSites.Count -eq 0) {
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'GetOneDriveSiteLockState' -Status 'NotFound' -Message 'No matching OneDrive personal site found for user.' -Data ([ordered]@{
        ...
        foreach ($site in @($resolvedSites | Sort-Object -Property Owner, Url)) {
            ...
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'GetOneDriveSiteLockState' -Status 'Completed' -Message $message -Data ([ordered]@{
                        ...
                        SiteOwner              = $siteOwner
                        LockState              = Get-SitePropertyValue -Site $site -PropertyNames @('LockState')
        ...
        $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'GetOneDriveSiteLockState' -Status 'Failed' -Message $_.Exception.Message -Data ([ordered]@{
                    UserPrincipalName      = $userPrincipalName
                    OneDriveUrl            = ''
                    ExpectedOneDriveUrl    = ''
                    SiteOwner              = ''
...
Write-Status -Message 'OneDrive lock state inventory script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
