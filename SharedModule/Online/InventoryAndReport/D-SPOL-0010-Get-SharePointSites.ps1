...
.EXAMPLE
    .\SM-IR3240-Get-SharePointSites.ps1 -InputCsvPath .\3240.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3240-Get-SharePointSites.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-SPOL-0010-Get-SharePointSites_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
