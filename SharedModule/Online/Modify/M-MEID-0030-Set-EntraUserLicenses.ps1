...
.SYNOPSIS
    Modifies EntraUserLicenses in Microsoft 365.

.DESCRIPTION
    Updates EntraUserLicenses in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3003-Set-EntraUserLicenses.ps1 -InputCsvPath .\3003.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3003-Set-EntraUserLicenses.ps1 -InputCsvPath .\3003.input.csv -WhatIf

    Dry-run: reports what would change without making any modifications.

.NOTES
    Version:          1.0
    Required modules: Microsoft.Graph.Authentication, Microsoft.Graph.Users, Microsoft.Graph.Users.Actions, Microsoft.Graph.Identity.DirectoryManagement
    ...
    SkuPartNumber         String    Yes       <fill in description>
    DisabledPlans         String    Yes       <fill in description>
#>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3003-Set-EntraUserLicenses_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
