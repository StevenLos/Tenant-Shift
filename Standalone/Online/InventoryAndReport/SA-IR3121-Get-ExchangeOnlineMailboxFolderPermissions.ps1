...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-IR3121-Get-ExchangeOnlineMailboxFolderPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
    ...
    try {
        ...
        foreach ($mailbox in @($mailboxes | Sort-Object -Property DisplayName, PrimarySmtpAddress)) {
            ...
            $folderStats = @(Invoke-WithRetry -OperationName "Load mailbox folders $mailboxIdentityResolved" -ScriptBlock {
                Get-EXOMailboxFolderStatistics -Identity $mailboxIdentityResolved -ErrorAction Stop
            })
            ...
            foreach ($folderStat in $folderStats) {
                ...
                $permissions = @(Invoke-WithRetry -OperationName "Load folder permissions $folderIdentity" -ScriptBlock {
                    Get-EXOMailboxFolderPermission -Identity $folderIdentity -ErrorAction SilentlyContinue
                })
