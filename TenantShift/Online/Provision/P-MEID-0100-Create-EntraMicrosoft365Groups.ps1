...
.SYNOPSIS
    Provisions EntraMicrosoft365Groups in Microsoft 365.

.DESCRIPTION
    Creates EntraMicrosoft365Groups in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P3008-Create-EntraMicrosoft365Groups.ps1 -InputCsvPath .\3008.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P3008-Create-EntraMicrosoft365Groups.ps1 -InputCsvPath .\3008.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P3008-Create-EntraMicrosoft365Groups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
