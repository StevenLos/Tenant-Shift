...
.EXAMPLE
    .\SM-IR3006-Get-EntraDynamicUserSecurityGroups.ps1 -InputCsvPath .\3006.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3006-Get-EntraDynamicUserSecurityGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-MEID-0080-Get-EntraDynamicUserSecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
