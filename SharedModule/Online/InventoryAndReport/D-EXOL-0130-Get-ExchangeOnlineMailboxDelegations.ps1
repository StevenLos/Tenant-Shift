...
.EXAMPLE
    .\SM-IR3120-Get-ExchangeOnlineMailboxDelegations.ps1 -InputCsvPath .\3120.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3120-Get-ExchangeOnlineMailboxDelegations.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0130-Get-ExchangeOnlineMailboxDelegations_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
