...
.EXAMPLE
    .\SM-IR3131-Get-ExchangeOnlineUserMailboxSmtpAddresses.ps1 -InputCsvPath .\3131.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3131-Get-ExchangeOnlineUserMailboxSmtpAddresses.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0260-Get-ExchangeOnlineUserMailboxSmtpAddresses_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
