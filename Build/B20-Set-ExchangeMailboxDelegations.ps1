...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path $PSScriptRoot -ChildPath ("Results_B20-Set-ExchangeMailboxDelegations_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting mailbox delegation assignment script.'
...
Write-Status -Message 'Mailbox delegation assignment script completed.' -Level SUCCESS
