...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-IR3125-Get-ExchangeOnlineRecipientTypeCounts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
    ...
    try {
        ...
        if ($recipientIdentityRaw -eq '*') {
            $allRecipients = @(Invoke-WithRetry -OperationName 'Load all recipients for type count inventory' -ScriptBlock {
                Get-EXORecipient -ResultSize Unlimited -ErrorAction Stop
            })
            ...
            foreach ($identity in $identities) {
                $resolved = Invoke-WithRetry -OperationName "Lookup recipient $identity" -ScriptBlock {
                    Get-EXORecipient -Identity $identity -ErrorAction SilentlyContinue
                }
