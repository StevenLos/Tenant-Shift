...
.SYNOPSIS
    Gets ExchangeOnPremOutlookClientVersionsFromRpcLogs and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremOutlookClientVersionsFromRpcLogs from Active Directory and writes the results to a CSV file.
...
.PARAMETER ClientSoftware
    Filter results to clients running a specific software string. Accepts wildcards.
...
.EXAMPLE
    .\SM-D0225-Get-ExchangeOnPremOutlookClientVersionsFromRpcLogs.ps1 -InputCsvPath .\0225.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0225-Get-ExchangeOnPremOutlookClientVersionsFromRpcLogs.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0225-Get-ExchangeOnPremOutlookClientVersionsFromRpcLogs_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
