...
.EXAMPLE
    .\SM-D3004-Get-EntraPrivilegedRoles.ps1 -InputCsvPath .\3004.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3004-Get-EntraPrivilegedRoles.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-MEID-0060-Get-EntraPrivilegedRoles_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
