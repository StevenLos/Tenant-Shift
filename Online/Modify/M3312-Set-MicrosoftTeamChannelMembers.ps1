...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_M3312-Set-MicrosoftTeamChannelMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($membershipType.Equals('Standard', [System.StringComparison]::OrdinalIgnoreCase)) {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey "$teamMailNickname|$channelDisplayName|$upn" -Action 'SetTeamChannelMembership' -Status 'Skipped' -Message 'Standard channels inherit Team membership. Use M3310-Set-MicrosoftTeamMembers.ps1 to manage access.'))
            $rowNumber++
            continue
        }
