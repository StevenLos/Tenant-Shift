...
.SYNOPSIS
    Modifies ExchangeOnPremDistributionListMembers in Active Directory.

.DESCRIPTION
    Updates ExchangeOnPremDistributionListMembers in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0215-Set-ExchangeOnPremDistributionListMembers.ps1 -InputCsvPath .\0215.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0215-Set-ExchangeOnPremDistributionListMembers.ps1 -InputCsvPath .\0215.input.csv -WhatIf
...
.NOTES
    ...
    MemberIdentity             String    Yes       <fill in description>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0215-Set-ExchangeOnPremDistributionListMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
