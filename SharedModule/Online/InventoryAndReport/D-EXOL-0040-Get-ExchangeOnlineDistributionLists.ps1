...
.EXAMPLE
    .\SM-IR3114-Get-ExchangeOnlineDistributionLists.ps1 -InputCsvPath .\3114.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3114-Get-ExchangeOnlineDistributionLists.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0040-Get-ExchangeOnlineDistributionLists_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
