...
.EXAMPLE
    .\SM-D3208-Get-OneDriveExternalSharingLinks.ps1 -InputCsvPath .\3208.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3208-Get-OneDriveExternalSharingLinks.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-ONDR-0050-Get-OneDriveExternalSharingLinks_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
