...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-IR3117-Get-ExchangeOnlineSharedMailboxPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
$resolveRecipientSummary = {
    ...
    try {
        $recipient = Invoke-WithRetry -OperationName "Lookup recipient $IdentityHint" -ScriptBlock {
            Get-EXORecipient -Identity $IdentityHint -ErrorAction Stop
        }
    ...
    try {
        ...
        if ($sharedMailboxIdentity -eq '*') {
            $mailboxes = @(Invoke-WithRetry -OperationName 'Load all shared mailboxes for permission inventory' -ScriptBlock {
                Get-EXOMailbox -RecipientTypeDetails SharedMailbox -ResultSize Unlimited -ErrorAction Stop
            })
        }
        else {
            $mailbox = Invoke-WithRetry -OperationName "Lookup mailbox $sharedMailboxIdentity" -ScriptBlock {
                Get-EXOMailbox -Identity $sharedMailboxIdentity -ErrorAction SilentlyContinue
            }
        ...
        foreach ($mailbox in @($mailboxes | Sort-Object -Property DisplayName, PrimarySmtpAddress)) {
            ...
            $mailboxPermissions = @(Invoke-WithRetry -OperationName "Load mailbox permissions $mailboxIdentityResolved" -ScriptBlock {
                Get-EXOMailboxPermission -Identity $mailboxIdentityResolved -ErrorAction Stop
            })

            foreach ($permission in $mailboxPermissions) {
                if ($permission.Deny -eq $true) {
                    continue
                }

                if ($permission.IsInherited -eq $true) {
                    continue
                }
            ...
            $recipientPermissions = @(Invoke-WithRetry -OperationName "Load recipient permissions $mailboxIdentityResolved" -ScriptBlock {
                Get-EXORecipientPermission -Identity $mailboxIdentityResolved -ErrorAction SilentlyContinue
            })
