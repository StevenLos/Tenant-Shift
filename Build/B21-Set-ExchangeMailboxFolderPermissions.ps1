...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path $PSScriptRoot -ChildPath ("Results_B21-Set-ExchangeMailboxFolderPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting mailbox folder permission assignment script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($isCalendarFolder -and ($calendarDelegate -or $calendarCanViewPrivateItems) -and (-not $addSupportsSharingFlags -or -not $setSupportsSharingFlags)) {
            throw 'This Exchange cmdlet session does not support -SharingPermissionFlags required for calendar delegate configuration.'
        }
...
Write-Status -Message 'Mailbox folder permission assignment script completed.' -Level SUCCESS
