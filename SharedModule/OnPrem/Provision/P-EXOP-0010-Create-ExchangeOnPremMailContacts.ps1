...
.SYNOPSIS
    Provisions ExchangeOnPremMailContacts in Active Directory.

.DESCRIPTION
    Creates ExchangeOnPremMailContacts in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P0213-Create-ExchangeOnPremMailContacts.ps1 -InputCsvPath .\0213.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P0213-Create-ExchangeOnPremMailContacts.ps1 -InputCsvPath .\0213.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P0213-Create-ExchangeOnPremMailContacts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
