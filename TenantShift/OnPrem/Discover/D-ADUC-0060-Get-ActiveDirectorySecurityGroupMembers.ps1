...
.EXAMPLE
    .\SM-D0007-Get-ActiveDirectorySecurityGroupMembers.ps1 -InputCsvPath .\0007.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0007-Get-ActiveDirectorySecurityGroupMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0007-Get-ActiveDirectorySecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
