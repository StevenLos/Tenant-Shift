...
.SYNOPSIS
    Provisions ExchangeOnlineDynamicDistributionGroups in Microsoft 365.

.DESCRIPTION
    Creates ExchangeOnlineDynamicDistributionGroups in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P3119-Create-ExchangeOnlineDynamicDistributionGroups.ps1 -InputCsvPath .\3119.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P3119-Create-ExchangeOnlineDynamicDistributionGroups.ps1 -InputCsvPath .\3119.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P3119-Create-ExchangeOnlineDynamicDistributionGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
