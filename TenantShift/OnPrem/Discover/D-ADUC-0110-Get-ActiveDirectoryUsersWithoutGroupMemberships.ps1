...
.SYNOPSIS
    Gets ActiveDirectoryUsersWithoutGroupMemberships and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectoryUsersWithoutGroupMemberships from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-D0011-Get-ActiveDirectoryUsersWithoutGroupMemberships.ps1 -InputCsvPath .\0011.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0011-Get-ActiveDirectoryUsersWithoutGroupMemberships.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0011-Get-ActiveDirectoryUsersWithoutGroupMemberships_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
