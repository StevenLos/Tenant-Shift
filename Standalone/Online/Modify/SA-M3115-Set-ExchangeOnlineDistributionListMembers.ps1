...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\\..\\Standalone_OutputCsvPath') -ChildPath ("Results_SA-M3115-Set-ExchangeOnlineDistributionListMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        $memberRecipient = Invoke-WithRetry -OperationName "Lookup recipient $memberUpn" -ScriptBlock {
            Get-EXORecipient -Identity $memberUpn -ErrorAction Stop
        }
