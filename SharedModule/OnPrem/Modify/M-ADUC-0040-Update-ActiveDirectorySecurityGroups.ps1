...
.SYNOPSIS
    Modifies ActiveDirectorySecurityGroups in Active Directory.

.DESCRIPTION
    Updates ActiveDirectorySecurityGroups in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0005-Update-ActiveDirectorySecurityGroups.ps1 -InputCsvPath .\0005.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0005-Update-ActiveDirectorySecurityGroups.ps1 -InputCsvPath .\0005.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0005-Update-ActiveDirectorySecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting Active Directory security group update script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($setParams.Count -gt 1) {
            if ($PSCmdlet.ShouldProcess($resolvedKey, 'Update Active Directory security group attributes')) {
                Invoke-WithRetry -OperationName "Update AD security group attributes $resolvedKey" -ScriptBlock {
        ...
        if ($changeCount -eq 0) {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $resolvedKey -Action 'UpdateActiveDirectorySecurityGroup' -Status 'Skipped' -Message 'No changes were requested for this row.'))
        }
        else {
            $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $resolvedKey -Action 'UpdateActiveDirectorySecurityGroup' -Status 'Completed' -Message ($messages -join ' ')))
        }
    }
    catch {
        Write-Status -Message "Row $rowNumber ($primaryKey) failed: $($_.Exception.Message)" -Level ERROR
        $results.Add((New-ResultObject -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'UpdateActiveDirectorySecurityGroup' -Status 'Failed' -Message $_.Exception.Message))
    }
...
Write-Status -Message 'Active Directory security group update script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
