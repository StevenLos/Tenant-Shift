...
.EXAMPLE
    .\SM-D3133-Get-ExchangeOnlineMailEnabledSecurityGroupMembers.ps1 -InputCsvPath .\3133.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3133-Get-ExchangeOnlineMailEnabledSecurityGroupMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0100-Get-ExchangeOnlineMailEnabledSecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
