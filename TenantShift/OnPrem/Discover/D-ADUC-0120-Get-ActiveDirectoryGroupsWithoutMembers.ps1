...
.SYNOPSIS
    Gets ActiveDirectoryGroupsWithoutMembers and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectoryGroupsWithoutMembers from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-D0012-Get-ActiveDirectoryGroupsWithoutMembers.ps1 -InputCsvPath .\0012.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0012-Get-ActiveDirectoryGroupsWithoutMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0012-Get-ActiveDirectoryGroupsWithoutMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
