...
.EXAMPLE
    .\SM-D3205-Get-OneDriveStorageAndQuota.ps1 -InputCsvPath .\3205.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3205-Get-OneDriveStorageAndQuota.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-ONDR-0020-Get-OneDriveStorageAndQuota_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
