...
.EXAMPLE
    .\SM-D0219-Get-ExchangeOnPremResourceMailboxBookingDelegates.ps1 -InputCsvPath .\0219.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0219-Get-ExchangeOnPremResourceMailboxBookingDelegates.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0219-Get-ExchangeOnPremResourceMailboxBookingDelegates_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
