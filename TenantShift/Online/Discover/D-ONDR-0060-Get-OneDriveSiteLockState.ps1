...
.EXAMPLE
    .\SM-D3209-Get-OneDriveSiteLockState.ps1 -InputCsvPath .\3209.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3209-Get-OneDriveSiteLockState.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-ONDR-0060-Get-OneDriveSiteLockState_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
