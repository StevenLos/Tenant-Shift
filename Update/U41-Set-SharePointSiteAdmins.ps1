...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path $PSScriptRoot -ChildPath ("Results_U41-Set-SharePointSiteAdmins_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)

$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$commonModulePath = Join-Path -Path (Split-Path -Parent $PSScriptRoot) -ChildPath 'Build\M365.Common.psm1'
