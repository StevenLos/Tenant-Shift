...
.EXAMPLE
    .\SM-D3128-Get-ExchangeOnlineMailboxStatsPerMailbox.ps1 -InputCsvPath .\3128.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3128-Get-ExchangeOnlineMailboxStatsPerMailbox.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0240-Get-ExchangeOnlineMailboxStatsPerMailbox_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
