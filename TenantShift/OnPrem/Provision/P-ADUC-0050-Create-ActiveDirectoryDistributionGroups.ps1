...
.SYNOPSIS
    Provisions ActiveDirectoryDistributionGroups in Active Directory.

.DESCRIPTION
    Creates ActiveDirectoryDistributionGroups in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-P0006-Create-ActiveDirectoryDistributionGroups.ps1 -InputCsvPath .\0006.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-P0006-Create-ActiveDirectoryDistributionGroups.ps1 -InputCsvPath .\0006.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_SM-P0006-Create-ActiveDirectoryDistributionGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
