...
.SYNOPSIS
    Gets ActiveDirectoryContacts and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectoryContacts from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-D0002-Get-ActiveDirectoryContacts.ps1 -InputCsvPath .\0002.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0002-Get-ActiveDirectoryContacts.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0002-Get-ActiveDirectoryContacts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
