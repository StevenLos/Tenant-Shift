...
.EXAMPLE
    .\SM-D3129-Get-ExchangeOnlineMailboxPermissionsConsolidated.ps1 -InputCsvPath .\3129.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3129-Get-ExchangeOnlineMailboxPermissionsConsolidated.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0250-Get-ExchangeOnlineMailboxPermissionsConsolidated_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
