...
.EXAMPLE
    .\SM-IR3002-Get-EntraGuestUsers.ps1 -InputCsvPath .\3002.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3002-Get-EntraGuestUsers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-MEID-0020-Get-EntraGuestUsers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
