# Operator Runbook — Online Discover Scripts
...
Resolve missing or outdated modules manually using the remediation command that `Initialize-TenantShift.ps1 -Profile OnlineOperator` prints. The script does not install or update modules automatically.
...
> **Warning:** Exchange Online commands use a default result limit that silently truncates output. Any D script that retrieves all mailboxes, recipients, or distribution group members must pass `-ResultSize Unlimited` to the underlying EXO command. Platform D scripts do this automatically when run in `-DiscoverAll` mode. If you are calling EXO commands directly or writing a custom script, you **must** include `-ResultSize Unlimited` or your results will be incomplete without any error or warning.

---

## 2. Preparing the Input CSV

D scripts support two modes: CSV-bounded (process only the objects listed in the CSV) and DiscoverAll (enumerate all objects in scope). For CSV-bounded runs, you need an input CSV.
...
```
.\TenantShift\Online\Discover\D-MEID-0010-Get-EntraUsers.ps1
.\TenantShift\Online\Discover\Scope-Users.input.csv    ← template
```
...
By default, results are written to a timestamped file under `Discover_OutputCsvPath\` in the script directory. To write results elsewhere:
...
```
.\TenantShift\Online\Discover\Discover_OutputCsvPath\
```
...
| `The term 'Get-EXOMailbox' is not recognized` | `ExchangeOnlineManagement` not installed or wrong version | Install or update `ExchangeOnlineManagement`, then re-run `Initialize-TenantShift.ps1 -Profile OnlineOperator` |
