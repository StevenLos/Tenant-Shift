...
.EXAMPLE
    .\SM-D3125-Get-ExchangeOnlineRecipientTypeCounts.ps1 -InputCsvPath .\3125.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3125-Get-ExchangeOnlineRecipientTypeCounts.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0210-Get-ExchangeOnlineRecipientTypeCounts_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
