...
.SYNOPSIS
    Gets ExchangeOnPremDistributionLists and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremDistributionLists from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0214-Get-ExchangeOnPremDistributionLists.ps1 -InputCsvPath .\0214.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0214-Get-ExchangeOnPremDistributionLists.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0214-Get-ExchangeOnPremDistributionLists_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
