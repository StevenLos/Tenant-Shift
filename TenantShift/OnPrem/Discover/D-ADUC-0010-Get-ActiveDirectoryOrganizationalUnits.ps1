...
.SYNOPSIS
    Gets ActiveDirectoryOrganizationalUnits and exports results to CSV.

.DESCRIPTION
    Gets ActiveDirectoryOrganizationalUnits from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-D0009-Get-ActiveDirectoryOrganizationalUnits.ps1 -InputCsvPath .\0009.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D0009-Get-ActiveDirectoryOrganizationalUnits.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_SM-D0009-Get-ActiveDirectoryOrganizationalUnits_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
