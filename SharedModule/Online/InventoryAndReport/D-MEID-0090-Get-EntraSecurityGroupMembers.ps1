...
.SYNOPSIS
    Gets EntraSecurityGroupMembers and exports results to CSV.

.DESCRIPTION
    Gets EntraSecurityGroupMembers from Microsoft 365 and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR3007-Get-EntraSecurityGroupMembers.ps1 -InputCsvPath .\3007.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3007-Get-EntraSecurityGroupMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-MEID-0090-Get-EntraSecurityGroupMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
