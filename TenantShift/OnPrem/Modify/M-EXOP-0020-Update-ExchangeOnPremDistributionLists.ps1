...
.SYNOPSIS
    Modifies ExchangeOnPremDistributionLists in Active Directory.

.DESCRIPTION
    Updates ExchangeOnPremDistributionLists in Active Directory based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M0214-Update-ExchangeOnPremDistributionLists.ps1 -InputCsvPath .\0214.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M0214-Update-ExchangeOnPremDistributionLists.ps1 -InputCsvPath .\0214.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M0214-Update-ExchangeOnPremDistributionLists_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
