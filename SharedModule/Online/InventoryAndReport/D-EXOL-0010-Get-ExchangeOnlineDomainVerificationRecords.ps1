...
.EXAMPLE
    .\SM-IR3124-Get-ExchangeOnlineDomainVerificationRecords.ps1 -InputCsvPath .\3124.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3124-Get-ExchangeOnlineDomainVerificationRecords.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0010-Get-ExchangeOnlineDomainVerificationRecords_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
