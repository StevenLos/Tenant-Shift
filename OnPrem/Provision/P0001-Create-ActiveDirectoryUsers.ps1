...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Provision_OutputCsvPath') -ChildPath ("Results_P0001-Create-ActiveDirectoryUsers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        Add-IfValue -Hashtable $newUserParams -Key 'EmployeeType' -Value $row.EmployeeType
