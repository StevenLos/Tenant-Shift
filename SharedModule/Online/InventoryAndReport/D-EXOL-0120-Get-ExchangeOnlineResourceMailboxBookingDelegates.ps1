...
.EXAMPLE
    .\SM-IR3119-Get-ExchangeOnlineResourceMailboxBookingDelegates.ps1 -InputCsvPath .\3119.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3119-Get-ExchangeOnlineResourceMailboxBookingDelegates.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0120-Get-ExchangeOnlineResourceMailboxBookingDelegates_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
