...
.EXAMPLE
    .\SM-D3121-Get-ExchangeOnlineMailboxFolderPermissions.ps1 -InputCsvPath .\3121.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3121-Get-ExchangeOnlineMailboxFolderPermissions.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-EXOL-0140-Get-ExchangeOnlineMailboxFolderPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
