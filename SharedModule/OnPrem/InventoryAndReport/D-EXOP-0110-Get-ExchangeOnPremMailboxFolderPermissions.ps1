...
.SYNOPSIS
    Gets ExchangeOnPremMailboxFolderPermissions and exports results to CSV.

.DESCRIPTION
    Gets ExchangeOnPremMailboxFolderPermissions from Active Directory and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR0221-Get-ExchangeOnPremMailboxFolderPermissions.ps1 -InputCsvPath .\0221.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR0221-Get-ExchangeOnPremMailboxFolderPermissions.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_SM-IR0221-Get-ExchangeOnPremMailboxFolderPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
