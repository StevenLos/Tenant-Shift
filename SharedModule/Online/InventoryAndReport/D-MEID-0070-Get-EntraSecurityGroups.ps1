...
.EXAMPLE
    .\SM-IR3005-Get-EntraSecurityGroups.ps1 -InputCsvPath .\3005.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3005-Get-EntraSecurityGroups.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-MEID-0070-Get-EntraSecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
