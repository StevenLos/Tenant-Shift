...
.SYNOPSIS
    Modifies ExchangeOnlineDynamicDistributionGroups in Microsoft 365.

.DESCRIPTION
    Updates ExchangeOnlineDynamicDistributionGroups in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3123-Update-ExchangeOnlineDynamicDistributionGroups.ps1 -InputCsvPath .\3123.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3123-Update-ExchangeOnlineDynamicDistributionGroups.ps1 -InputCsvPath .\3123.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3123-Update-ExchangeOnlineDynamicDistributionGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
