...
.SYNOPSIS
    Gets OneDriveSharingSettings and exports results to CSV.

.DESCRIPTION
    Gets OneDriveSharingSettings from Microsoft 365 and writes the results to a CSV file.
...
.EXAMPLE
    .\SM-IR3207-Get-OneDriveSharingSettings.ps1 -InputCsvPath .\3207.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3207-Get-OneDriveSharingSettings.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-ONDR-0040-Get-OneDriveSharingSettings_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
Write-Status -Message 'Starting OneDrive sharing settings inventory script.'
...
foreach ($row in $rows) {
    ...
    try {
        ...
        if ($resolvedSites.Count -eq 0) {
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'GetOneDriveSharingSettings' -Status 'NotFound' -Message 'No matching OneDrive personal site found for user.' -Data ([ordered]@{
        ...
        foreach ($site in @($resolvedSites | Sort-Object -Property Owner, Url)) {
            ...
            $message = if ($userPrincipalName -ne '*' -and $resolvedSites.Count -gt 1) {
                ...
                'OneDrive sharing settings exported.'
            }
            ...
            $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $primaryKey -Action 'GetOneDriveSharingSettings' -Status 'Completed' -Message $message -Data ([ordered]@{
                        ...
                        SharingDomainRestrictionMode     = Get-SitePropertyValue -Site $site -PropertyNames @('SharingDomainRestrictionMode')
                        SharingAllowedDomainList         = Get-SitePropertyValue -Site $site -PropertyNames @('SharingAllowedDomainList')
                        SharingBlockedDomainList         = Get-SitePropertyValue -Site $site -PropertyNames @('SharingBlockedDomainList')
                        DefaultSharingLinkType           = Get-SitePropertyValue -Site $site -PropertyNames @('DefaultSharingLinkType')
                        DefaultLinkPermission            = Get-SitePropertyValue -Site $site -PropertyNames @('DefaultLinkPermission')
                        DefaultLinkToExistingAccess      = Get-SitePropertyValue -Site $site -PropertyNames @('DefaultLinkToExistingAccess')
                        DisableCompanyWideSharingLinks   = Get-SitePropertyValue -Site $site -PropertyNames @('DisableCompanyWideSharingLinks')
                        FileAnonymousLinkType            = Get-SitePropertyValue -Site $site -PropertyNames @('FileAnonymousLinkType')
                        FolderAnonymousLinkType          = Get-SitePropertyValue -Site $site -PropertyNames @('FolderAnonymousLinkType')
                    })))
        }
    }
    catch {
        Write-Status -Message "Row $rowNumber ($userPrincipalName) failed: $($_.Exception.Message)" -Level ERROR
        $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey $userPrincipalName -Action 'GetOneDriveSharingSettings' -Status 'Failed' -Message $_.Exception.Message -Data ([ordered]@{
                    UserPrincipalName                = $userPrincipalName
                    OneDriveUrl                      = ''
                    ExpectedOneDriveUrl              = ''
                    SiteOwner                        = ''
...
Write-Status -Message 'OneDrive sharing settings inventory script completed.' -Level SUCCESS
}
finally {
    Stop-RunTranscript
}
