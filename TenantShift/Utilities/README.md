# TenantShift Utilities

`TenantShift/Utilities/` contains helper scripts that support TenantShift operation and are not tied to a single workload folder.

## Purpose

- Host one-off or cross-workload utility scripts that operate within the TenantShift context.
...
```powershell
pwsh ./TenantShift/Utilities/Utility-Generate-Passwords.ps1 -InputCsvPath ./TenantShift/Utilities/Utility-Generate-Passwords.input.csv -PasswordCount 20
```

Password generator with CSV-controlled count:

```powershell
pwsh ./TenantShift/Utilities/Utility-Generate-Passwords.ps1 -InputCsvPath ./TenantShift/Utilities/Utility-Generate-Passwords.input.csv
```

Espresso utility (run until interrupted):

```powershell
pwsh ./TenantShift/Utilities/Utility-Espresso.ps1
```

Espresso utility with finite duration:

```powershell
pwsh ./TenantShift/Utilities/Utility-Espresso.ps1 -DurationMinutes 120 -IntervalSeconds 59
```

CSV folder to single XLSX workbook:

```powershell
pwsh ./TenantShift/Utilities/Utility-Convert-CsvFolderToSingleXlsx.ps1 -InputFolderPath ./TenantShift/Online/Discover/Discover_OutputCsvPath
```

CSV folder to single XLSX workbook with recursion and overwrite:

```powershell
pwsh ./TenantShift/Utilities/Utility-Convert-CsvFolderToSingleXlsx.ps1 -InputFolderPath ./TenantShift/Online/Discover -Recurse -OutputXlsxPath ./TenantShift/Online/Discover/InventoryWorkbook.xlsx -Overwrite
```

CSV folder to individual XLSX files:

```powershell
pwsh ./TenantShift/Utilities/Utility-Convert-CsvFolderToIndividualXlsx.ps1 -InputFolderPath ./TenantShift/Online/Discover/Discover_OutputCsvPath
```

CSV folder to individual XLSX files with recursion and separate output folder:

```powershell
pwsh ./TenantShift/Utilities/Utility-Convert-CsvFolderToIndividualXlsx.ps1 -InputFolderPath ./TenantShift/Online/Discover -Recurse -OutputFolderPath ./TenantShift/Online/Discover/ConvertedXlsx -Overwrite
```

Repository prerequisite scan:

```powershell
pwsh ./TenantShift/Utilities/Utility-Check-Prerequisites.ps1 -RepositoryRoot . -SkipGalleryCheck
```

Repository prerequisite scan with Online operator validation and CI-style fail conditions:

```powershell
pwsh ./TenantShift/Utilities/Utility-Check-Prerequisites.ps1 -RepositoryRoot . -Profile OnlineOperator -FailOn Missing,ProfileFailure
```

Recursively unblock downloaded files under a folder:

```powershell
pwsh ./TenantShift/Utilities/Utility-Unblock-Files.ps1 -TargetPath ./TenantShift
```
