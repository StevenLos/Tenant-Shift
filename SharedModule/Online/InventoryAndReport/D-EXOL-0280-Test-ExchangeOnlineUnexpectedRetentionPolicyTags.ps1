...
.EXAMPLE
    .\SM-IR3130-Test-ExchangeOnlineUnexpectedRetentionPolicyTags.ps1 -InputCsvPath .\3130.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3130-Test-ExchangeOnlineUnexpectedRetentionPolicyTags.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0280-Test-ExchangeOnlineUnexpectedRetentionPolicyTags_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
