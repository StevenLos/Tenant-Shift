...
.EXAMPLE
    .\SM-D3312-Get-MicrosoftTeamChannelMembers.ps1 -InputCsvPath .\3312.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3312-Get-MicrosoftTeamChannelMembers.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-TEAM-0040-Get-MicrosoftTeamChannelMembers_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
        ...
        foreach ($group in @($candidateGroups | Sort-Object -Property MailNickname, DisplayName, Id)) {
            ...
            foreach ($channel in @($selectedChannels | Sort-Object -Property DisplayName, Id)) {
                ...
                if ($membershipType.Equals('Standard', [System.StringComparison]::OrdinalIgnoreCase)) {
                    $results.Add((New-InventoryResult -RowNumber $rowNumber -PrimaryKey "$groupAlias|$channelId" -Action 'GetMicrosoftTeamChannelMember' -Status 'Completed' -Message 'Standard channels inherit Team membership; use D3310 for Team-level members.' -Data ([ordered]@{
                                TeamGroupId                  = $groupId
                                TeamDisplayName              = $groupName
                                TeamMailNickname             = $groupAlias
                                ChannelId                    = $channelId
