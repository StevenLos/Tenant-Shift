...
.SYNOPSIS
    Modifies ExchangeOnlineMailboxFolderPermissions in Microsoft 365.

.DESCRIPTION
    Updates ExchangeOnlineMailboxFolderPermissions in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3121-Set-ExchangeOnlineMailboxFolderPermissions.ps1 -InputCsvPath .\3121.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3121-Set-ExchangeOnlineMailboxFolderPermissions.ps1 -InputCsvPath .\3121.input.csv -WhatIf
...
.NOTES
    ...
    FolderPath                   String    Yes       <fill in description>
    AccessRights                 String    Yes       <fill in description>
    CalendarDelegate             String    Yes       <fill in description>
    CalendarCanViewPrivateItems  String    Yes       <fill in description>
    SendNotificationToUser       String    Yes       <fill in description>
#>
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3121-Set-ExchangeOnlineMailboxFolderPermissions_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
...
foreach ($row in $rows) {
    ...
    try {
        ...
        $existingPermissions = @(Invoke-WithRetry -OperationName "Check folder permission $folderIdentity -> $trusteeUpn" -ScriptBlock {
            Get-ExchangeOnlineMailboxFolderPermission -Identity $folderIdentity -User $trustee.Identity -ErrorAction SilentlyContinue
        })
