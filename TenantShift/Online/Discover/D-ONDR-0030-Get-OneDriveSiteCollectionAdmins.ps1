...
.EXAMPLE
    .\SM-D3206-Get-OneDriveSiteCollectionAdmins.ps1 -InputCsvPath .\3206.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-D3206-Get-OneDriveSiteCollectionAdmins.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Discover_OutputCsvPath') -ChildPath ("Results_D-ONDR-0030-Get-OneDriveSiteCollectionAdmins_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
