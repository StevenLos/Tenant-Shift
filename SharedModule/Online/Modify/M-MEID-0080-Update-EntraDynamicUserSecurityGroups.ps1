...
.SYNOPSIS
    Modifies EntraDynamicUserSecurityGroups in Microsoft 365.

.DESCRIPTION
    Updates EntraDynamicUserSecurityGroups in Microsoft 365 based on records provided in the input CSV file.
...
.EXAMPLE
    .\SM-M3006-Update-EntraDynamicUserSecurityGroups.ps1 -InputCsvPath .\3006.input.csv

    Process all records in the input CSV file.

.EXAMPLE
    .\SM-M3006-Update-EntraDynamicUserSecurityGroups.ps1 -InputCsvPath .\3006.input.csv -WhatIf
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'Modify_OutputCsvPath') -ChildPath ("Results_SM-M3006-Update-EntraDynamicUserSecurityGroups_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
