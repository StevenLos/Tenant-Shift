...
.EXAMPLE
    .\SM-IR3126-Get-ExchangeOnlineMailboxHighLevelStats.ps1 -InputCsvPath .\3126.input.csv

    Inventory the objects listed in the input CSV file.

.EXAMPLE
    .\SM-IR3126-Get-ExchangeOnlineMailboxHighLevelStats.ps1 -DiscoverAll
...
param(
    ...
    [string]$OutputCsvPath = (Join-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath 'InventoryAndReport_OutputCsvPath') -ChildPath ("Results_D-EXOL-0220-Get-ExchangeOnlineMailboxHighLevelStats_{0}.csv" -f (Get-Date -Format 'yyyyMMdd-HHmmss')))
)
